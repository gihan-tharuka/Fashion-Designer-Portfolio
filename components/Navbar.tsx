...
export function Navbar() {
  ...
  return (
    <>
      <header
        className={`sticky top-0 z-[300] transition duration-500 ${
          isOpen
            ? "border-b border-gold/18 bg-ivory shadow-[0_10px_40px_rgba(58,36,24,0.05)]"
            : hasScrolled
      ...
      >
        <nav className="editorial-container relative z-[320] flex min-h-16 items-center justify-between py-3">
          ...
          <button
            type="button"
            className="focus-ring relative z-[330] inline-flex h-11 w-11 items-center justify-center rounded-full border border-gold/30 bg-cream text-brown transition duration-500 hover:border-gold hover:bg-cream/80 md:hidden"
              ...
              {navItems.map((item, index) => {
                ...
                return (
                  <Link
                    key={item.href}
                    href={item.href}
                    className={`mobile-menu-link focus-ring serif block w-full border-b border-gold/18 py-4 text-5xl font-semibold leading-none transition duration-500 ${
            ...
            <div className="grid gap-5 border-t border-gold/24 pt-7">
              <div className="grid gap-2 text-sm leading-7 text-muted">
                <a
                  ...
                  className="block py-1 transition hover:text-espresso"
                >
                  imalkatharuni24@gmail.com
                </a>
                <a href="tel:+94782870261" className="block py-1 transition hover:text-espresso">
                  078 287 0261
                </a>
              </div>
              <Link
